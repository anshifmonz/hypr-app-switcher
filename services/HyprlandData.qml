    ...
    Connections {
        ...
        function onRawEvent(event) {
            ...
            if (eventName === "activespecial") {
                ...
                root.activeSpecialName = parts[0] || "";
                if (!root.activeSpecialName) {
                    root.activeSpecialId = 0;
                }
                scheduleUpdates(true, true, false, true, true);
                return;
            }

            if (eventName === "activespecialv2") {
                const parts = (event.data || "").split(",");
                root.activeSpecialId = parseInt(parts[0]) || 0;
                root.activeSpecialName = parts[1] || "";
                scheduleUpdates(true, true, false, true, true);
                return;
            }
        ...
        stdout: StdioCollector {
            ...
            onStreamFinished: {
                ...
                for (var i = 0; i < root.monitors.length; i++) {
                    ...
                    if (m && m.focused && m.specialWorkspace && m.specialWorkspace.id !== 0) {
                        root.activeSpecialId = m.specialWorkspace.id;
                        root.activeSpecialName = m.specialWorkspace.name;
                        break;
                    }
                }
            }
